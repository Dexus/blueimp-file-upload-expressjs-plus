    ...
    var api = {
        ...
        proccessVersionFile: function(versionObj, cbk) {
            ...
            lwip.open(options.uploadDir + '/' + versionObj.fileInfo.name, function(error, image) {
                ...
                if ( opts0.maxDimension ) {
                    
                    if ( image.width() > image.height() ) {
                        retVal.width = opts0.maxDimension;
                        if ( opts0.upscale === false ) {
                            if ( retVal.width > image.width() ) retVal.width = image.width();
                        }
                        retVal.height = (retVal.width / retVal.fileInfo.width) * image.height();
                    } else {
                        retVal.height = opts0.maxDimension;
                        if ( opts0.upscale === false ) {
                            if ( retVal.height > image.height() ) retVal.height = image.height();
                        }
                        retVal.width = (retVal.height / retVal.fileInfo.height) * image.width();
                    }

                    image.batch().resize(retVal.width, retVal.height).writeFile(options.uploadDir + '/' + versionObj.version + '/' + versionObj.fileInfo.name, function(err) {
                        if (err) {
                            cbk(err, retVal);
                            return;
                        }
                ...
                if (opts0.width == 'auto') {
                    ...
                    retVal.width = (opts0.height / retVal.fileInfo.height) * retVal.fileInfo.width;
                    image.batch().resize(retVal.width, retVal.height).writeFile(options.uploadDir + '/' + versionObj.version + '/' + versionObj.fileInfo.name, function(err) {
                        if (err) {
                            cbk(err, retVal);
                            return;
        ...
        post: function(fileInfo, file, finish) {
            ...
            if ((versions.length===0) || (!options.imageTypes.test(fileInfo.name))) {
                fileInfo.initUrls();
            ...
            async.map(versionFuncs, me.proccessVersionFile, function(err, results) {
                ...
                if ( options.saveOriginal === false ) {
                    fs.unlinkSync( options.uploadDir + '/' + fileInfo.name);
                }
